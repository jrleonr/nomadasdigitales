<div class="px-4 py-16 sm:px-6 sm:py-24 md:grid md:place-items-center lg:px-8">
    <div class="mx-auto max-w-4xl ">
      <main class="sm:flex">
        <p class="text-4xl font-bold tracking-tight text-indigo-600 sm:text-5xl">Último paso</p>
        <div class="sm:ml-6">
          <div class="sm:border-l sm:border-gray-200 sm:pl-6">
            <h1 class="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">Verifica tu email</h1>
            <p class="mt-1 text-base text-gray-500">
                Vete a tu correo electrónico y haz clic en el botón en el correo que te hemos enviado. 
                Queremos validar que eres tú. Una vez hecho, podrás verificar tu oferta una vez más antes de enviarla. 
                Una vez hecho, podremos enviar tu propuesta a las agencias para que encuentres ayuda lo antes posible.
            </p>
          </div>
        </div>
      </main>
    </div>
  </div>